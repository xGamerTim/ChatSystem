...
public class ArangoMethods {
    ...
    public static void setGlobalChatBoolean(String uuid){
        if(ArangoUtils.arangoDB.db(database).collection(Collection).getDocument(uuid.toString(), BaseDocument.class) == null){
            BaseDocument base = new BaseDocument();
            base.addAttribute("name", Bukkit.getPlayer(uuid).getName());
